...
interface Result {
  ...
  cms_list?: string; wcms_name?: string; osearch_group?: string
  osearch?: string; ems_list?: string; ai_category?: string
  ai_is_ecommerce?: string; ai_industry?: string; bw_vertical?: string
  sw_category?: string; sw_subcategory?: string; sw_description?: string
  sw_title?: string; company_name?: string; sw_primary_region?: string
  sw_primary_region_pct?: number; error_detail?: string
}
interface Credits { similarweb?: number | null; builtwith?: number | null }
interface StatsItem { name: string; count: number }
...
function fmt(v?: number | null) { return v != null ? v.toLocaleString("en-US") : "—" }
        ...
        <div className="dialog-body">
          {services.includes("builtwith") && (
            <div className={`dialog-row ${!bwOk ? "dialog-row-error" : ""}`}>
              ...
              <span className="dialog-stat">Залишок: <b>{fmt(credits.builtwith)}</b></span>
          ...
          {services.includes("similarweb") && (
            <div className={`dialog-row ${!swOk ? "dialog-row-error" : ""}`}>
              ...
              <span className="dialog-stat">Залишок: <b>{fmt(credits.similarweb)}</b></span>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string,string> = { pending:"badge-pending", running:"badge-running", completed:"badge-completed", completed_with_errors:"badge-warn", failed:"badge-failed", cancelled:"badge-cancelled" }
  return <span className={`badge ${map[status]||"badge-pending"}`}>{status}</span>
}

function ProgressBar({ value, total, showPct = true }: { value: number; total: number; showPct?: boolean }) {
  const pct = total > 0 ? Math.round((value / total) * 100) : 0
  return (
    <div className="progress-wrap">
      <div className="progress-track"><div className="progress-fill" style={{ width: `${pct}%` }} /></div>
      <span className="progress-label">{value}/{total}{showPct ? ` (${pct}%)` : ""}</span>
    </div>
  )
}
...
function NewJobPage({ onJobCreated }: { onJobCreated: (id: string) => void }) {
  ...
  const loadCredits = useCallback(async () => { try { setCredits(await apiFetch("/api/credits")) } catch {} }, [])
  ...
  const serviceOptions = [
    ...
    { id:"whatcms",    label:"WhatCMS",    sublabel:"підтвердження CMS" },
  ...
  return (
    <div className="page">
      ...
      <div className="card">
        ...
        <textarea className="manual-domains" placeholder={"один домен на рядок\nexample.com\nshop.example.ua"} value={manualDomains} onChange={e=>handleManualChange(e.target.value)} rows={5} />
      ...
      <div className="card">
        ...
        <div className={`dropzone ${dragging?"dragging":""} ${file?"has-file":""}`}
          ...
          <input id="file-input" type="file" accept=".csv,.xlsx,.xls,.txt" style={{display:"none"}} onChange={e=>e.target.files?.[0]&&handleFile(e.target.files[0])} />
          {file ? (
      ...
      <button className={`btn-primary ${loading?"loading":""}`} onClick={handleSubmitClick} disabled={loading||(!file&&!manualDomains.trim())||services.length===0}>
...
function JobsPage({ onSelect }: { onSelect: (id:string)=>void }) {
  ...
  const load = useCallback(async()=>{ try{const d=await apiFetch("/api/jobs");setJobs(d.jobs)}catch{}finally{setLoading(false)} },[])
  ...
  return (
    <div className="page">
      ...
      {loading?<div className="loading-center"><span className="spinner-lg"/></div>
        :jobs.length===0?<div className="empty-state">Ще немає job-ів. Запустіть перший аналіз.</div>
        :(
          <div className="jobs-list">
            {jobs.map(job=>(
              <div key={job.job_id} className="job-card" onClick={()=>onSelect(job.job_id)}>
                <div className="job-card-header">
                  <div><div className="job-filename">{job.filename}</div>
                  <div className="job-meta"><span className="job-id">{job.job_id.slice(0,8)}</span><span className="job-date">{new Date(job.created_at).toLocaleString("uk-UA")}</span></div></div>
                  <StatusBadge status={job.status}/>
                </div>
                <div className="job-services">{job.services?.map(s=><span key={s} className="service-tag">{s}</span>)}</div>
                <ProgressBar value={(job.processed_domains||0)+(job.failed_domains||0)} total={job.total_domains||0}/>
                {job.failed_domains>0&&<div className="job-errors">{job.failed_domains} помилок</div>}
              </div>
            ))}
          </div>
        )}
...
const COLUMNS = [
  ...
  {key:"cms_list",label:"CMS",w:"120px"},{key:"wcms_name",label:"WhatCMS",w:"110px"},
  {key:"osearch_group",label:"oSearch Group",w:"120px"},{key:"osearch",label:"oSearch",w:"130px"},
  {key:"ems_list",label:"EMS",w:"120px"},{key:"ai_category",label:"AI Category",w:"140px"},
...
function ResultsPage({ jobId, onBack }: { jobId:string; onBack:()=>void }) {
  ...
  useEffect(()=>{
    loadData()
    const iv=setInterval(()=>{ if(job?.status==="running"||job?.status==="pending") loadData() },5000)
    return()=>clearInterval(iv)
  },[loadData,job?.status])

  // Poll for sheets URL after export
  const pollSheetsUrl = useCallback(async()=>{
    for(let i=0;i<10;i++){
      await new Promise(r=>setTimeout(r,2000))
      const data = await apiFetch(`/api/jobs/${jobId}/export/sheets/url`)
  ...
  const handleExportSheets = async()=>{
    ...
    await apiFetch(`/api/jobs/${jobId}/export/sheets`,{method:"POST"})
    pollSheetsUrl()
  }
  ...
  const renderCell=(r:Result,key:string)=>{
    ...
    <div className="page page-wide">
      <div className="page-header">
        ...
        <div className="page-header-actions">
          ...
          {sheetsUrl
            ? <a href={sheetsUrl} target="_blank" rel="noopener" className="btn-export btn-sheets">📊 Sheets</a>
            : <button className="btn-export btn-sheets" onClick={handleExportSheets} disabled={sheetsLoading}>
      ...
      {job&&(job.status==="running"||job.status==="pending")&&(
        <div className="progress-banner"><ProgressBar value={(job.processed_domains||0)+(job.failed_domains||0)} total={job.total_domains||0}/></div>
      )}

      <div className="filter-row">
        <input className="filter-input" placeholder="Фільтр по домену, CMS, категорії..." value={filter} onChange={e=>setFilter(e.target.value)}/>
          ...
          <tbody>
            {filtered.map(r=>(
              <tr key={r.domain} className={r.status==="error"?"row-error":""}>
                {COLUMNS.map(c=>(
                  <td key={c.key} className={c.key==="domain"?"td-domain":c.key==="sw_visits"?"td-traffic":c.key==="sw_description"?"td-desc":""}
                    title={c.key==="sw_description"?cell((r as any)[c.key]):undefined}>
                    {c.key==="domain"?<a href={`https://${r.domain}`} target="_blank" rel="noopener">{r.domain}</a>:renderCell(r,c.key)}
                  </td>
                ))}
              </tr>
            ))}
