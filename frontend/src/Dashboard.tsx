// Dashboard.tsx — Interactive charts reacting to Explorer filters
...
function getGroup(v?:number){ if(!v||v<=0)return "Nano <10k"; for(const g of TRAFFIC_GROUPS)if(v>=g.min)return g.label; return "Nano <10k" }

function flag(code:string){ if(!code||code.length!==2)return "🌍"; return String.fromCodePoint(...code.toUpperCase().split("").map(c=>127397+c.charCodeAt(0))) }
    ...
    <div className="dash-chart">
      ...
      <div className="dash-chart-body">
        <svg width={sz} height={sz} style={{flexShrink:0}}>
          {slices.map((s,i)=><path key={i} d={s.d} fill={s.color} stroke="var(--bg)" strokeWidth={1.5}><title>{s.label}: {s.count.toLocaleString()} ({(s.pct*100).toFixed(1)}%)</title></path>)}
...
export default function Dashboard({profiles}:{profiles:Profile[]}){
  const charts=useMemo(()=>{
    const trafficMap=new Map<string,number>(); TRAFFIC_GROUPS.forEach(g=>trafficMap.set(g.label,0))
    ...
    for(const p of profiles){
      const g=getGroup(p.sw_visits); trafficMap.set(g,(trafficMap.get(g)||0)+1)
    ...
    const trafficData:Slice[]=TRAFFIC_GROUPS.map((g,i)=>({label:g.label,count:trafficMap.get(g.label)||0,color:g.color})).filter(d=>d.count>0)
